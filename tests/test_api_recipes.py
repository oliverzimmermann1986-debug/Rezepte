...
def test_recipe_comments_translate_once_cache_and_keep_original(
    client, test_db, monkeypatch
):
    import app.routes.api_recipes as api_recipes
    from app.core.analyzer import TextTranslation

    recipe = _create_recipe(
        test_db, name="Kommentar-Test", folder_path="/tmp/comments"
    )
    monkeypatch.setattr(api_recipes, "_actor", lambda _request: "Anna")

    created = client.post(
        f"/api/recipes/{recipe['id']}/comments",
        json={"body": "Beim nächsten Mal mehr Knoblauch.", "source_language": "de"},
    )
    ...
    class Analyzer:
        ...
        def translate_text(self, text, language):
            self.calls += 1
            assert text == "Beim nächsten Mal mehr Knoblauch."
            assert language == "en"
            return TextTranslation(
                text="Use more garlic next time.",
                translated=True,
                source_language="de",
            )

    analyzer = Analyzer()
    monkeypatch.setattr(api_recipes, "build_analyzer", lambda _config: analyzer)

    first = client.get(
        f"/api/recipes/{recipe['id']}/comments", params={"language": "en"}
    )
    second = client.get(
        f"/api/recipes/{recipe['id']}/comments", params={"language": "en"}
    )
    ...
    assert item["text"] == "Use more garlic next time."
    assert item["original_text"] == "Beim nächsten Mal mehr Knoblauch."
...
def test_recipe_comments_fall_back_to_original_when_translation_fails(
    client, test_db, monkeypatch
):
    import app.routes.api_recipes as api_recipes

    recipe = _create_recipe(
        test_db, name="Fallback-Test", folder_path="/tmp/comments-fallback"
    )
    monkeypatch.setattr(api_recipes, "_actor", lambda _request: "Oliver")
    client.post(
        f"/api/recipes/{recipe['id']}/comments",
        json={"body": "Schmeckt am zweiten Tag besser.", "source_language": "de"},
    )

    class Analyzer:
        def translate_text(self, _text, _language):
            return None

    monkeypatch.setattr(api_recipes, "build_analyzer", lambda _config: Analyzer())
    response = client.get(
        f"/api/recipes/{recipe['id']}/comments", params={"language": "fr"}
    )

    assert response.status_code == 200
    item = response.json()["items"][0]
    assert item["text"] == "Schmeckt am zweiten Tag besser."
    assert item["original_text"] == item["text"]
    assert item["translation_status"] == "unavailable"


def test_recipe_comments_only_author_can_delete(client, test_db, monkeypatch):
    import app.routes.api_recipes as api_recipes

    recipe = _create_recipe(
        test_db, name="Lösch-Test", folder_path="/tmp/comments-delete"
    )
    actor = {"name": "Anna"}
    monkeypatch.setattr(api_recipes, "_actor", lambda _request: actor["name"])
    created = client.post(
        f"/api/recipes/{recipe['id']}/comments",
        json={"body": "Meine Notiz", "source_language": "de"},
    ).json()

    actor["name"] = "Oliver"
    denied = client.delete(
        f"/api/recipes/{recipe['id']}/comments/{created['comment_id']}"
    )
...
def test_recipe_comments_reject_unknown_language(client, test_db):
    recipe = _create_recipe(
        test_db, name="Sprach-Test", folder_path="/tmp/comments-language"
    )

    assert client.get(
        f"/api/recipes/{recipe['id']}/comments", params={"language": "xx"}
    ).status_code == 422
    assert client.post(
        f"/api/recipes/{recipe['id']}/comments",
        json={"body": "Notiz", "source_language": "xx"},
    ).status_code == 422
