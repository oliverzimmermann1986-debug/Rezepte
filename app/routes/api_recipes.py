...
COMMENT_LANGUAGES = frozenset({"de", "en", "fr", "es", "it", "nl"})


def _comment_language(value: str) -> str:
    language = (value or "").strip().lower()
    if language not in COMMENT_LANGUAGES:
        raise HTTPException(422, "Nicht unterstützte Kommentarsprache")
    return language


class RecipeCommentCreate(BaseModel):
    body: str = Field(min_length=1, max_length=2_000)
    source_language: str = Field(default="de", min_length=2, max_length=5)


def _comment_response(
    comment: Dict[str, Any], *, text: str, target_language: str,
    translated: bool, translation_status: str, actor: str,
    ...
    return {
        ...
        "text": text,
        "source_language": comment.get("source_language") or "de",
        ...
        "can_delete": str(comment["created_by"]).casefold() == actor.casefold(),
    }
...
def recipe_comments(
    recipe_id: int,
    request: Request,
    language: str = Query("de", min_length=2, max_length=5),
) -> Dict[str, Any]:
    """Liefert Kochnotizen in der Gerätesprache, mit Original als Fallback."""
    db = get_db()
    recipe = db.recipe_get(recipe_id)
    if not recipe or recipe.get("deleted_at") is not None:
        raise HTTPException(404, "Rezept nicht gefunden")
    target_language = _comment_language(language)
    actor = _actor(request)
    analyzer = None
    analyzer_unavailable = False
    items: List[Dict[str, Any]] = []

    for comment in db.recipe_comments_list(recipe_id):
        original = str(comment.get("body") or "")
        source_language = _comment_language(
            str(comment.get("source_language") or "de")
        )
        cached = db.recipe_comment_translation_get(
            int(comment["id"]), target_language
        )
        if cached:
            items.append(_comment_response(
                comment,
                text=str(cached["translated_text"]),
                target_language=target_language,
                translated=bool(cached.get("is_translated")),
                translation_status=(
                    "translated" if cached.get("is_translated") else "original"
                ),
                actor=actor,
                detected_source_language=cached.get("detected_source_language"),
            ))
            continue

        if analyzer is None and not analyzer_unavailable:
            try:
                analyzer = build_analyzer(
                    get_config().get("ai", default={}) or {}
                )
            except ValueError as exc:
                analyzer_unavailable = True
                logger.warning("Kommentarübersetzung ist nicht konfiguriert: %s", exc)

        translation = None
        if analyzer is not None:
            try:
                translation = analyzer.translate_text(original, target_language)
            except Exception:
                logger.exception(
                    "Kommentar #%s konnte nicht nach %s übersetzt werden",
                    comment["id"], target_language,
                )
        ...
        db.recipe_comment_translation_set(
            int(comment["id"]),
            target_language,
            translation.text,
            is_translated=translation.translated,
        ...
        items.append(_comment_response(
            ...
            translation_status=(
                "translated" if translation.translated else "original"
            ),
            actor=actor,
            detected_source_language=translation.source_language,
        ))

    return {"items": items, "language": target_language}


@router.post("/{recipe_id}/comments", status_code=201)
def create_recipe_comment(
    recipe_id: int, payload: RecipeCommentCreate, request: Request
) -> Dict[str, Any]:
    db = get_db()
    recipe = db.recipe_get(recipe_id)
    if not recipe or recipe.get("deleted_at") is not None:
    ...
    if not body:
        raise HTTPException(422, "Kommentar darf nicht leer sein")
    source_language = _comment_language(payload.source_language)
    comment = db.recipe_comment_create(
        recipe_id,
        body,
        source_language=source_language,
        created_by=_actor(request),
    )
    return {"ok": True, "comment_id": int(comment["id"])}


@router.delete("/{recipe_id}/comments/{comment_id}")
def delete_recipe_comment(
    recipe_id: int, comment_id: int, request: Request
) -> Dict[str, Any]:
    db = get_db()
    if not db.recipe_comment_delete(
        recipe_id, comment_id, created_by=_actor(request)
    ):
